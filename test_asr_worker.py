...
async def test_typed_finalization_and_late_interims():
    ...
    result = await w.finalize_turn(timeout=2)
    w.stop()
    check("interim-only flush is never labelled final", result.status == "empty" and not result.text and result.interim == "sofa ka")
...
async def test_real_call_loop_with_buffered_turns():
    ...
    class FakeVAD:
        ...
        def process_chunk(self, samples, **kw):
            assert not agent.is_speaking, "queued caller audio processed during reply"
            assert not playback.is_playing(), "queued caller audio processed during playback"
            self.index += 1
            return SimpleNamespace(
                is_speech=True, speech_start=self.index % 2 == 1,
                speech_probability=0.9, barge_in=False,
    ...
    module.sd.query_devices.return_value = {"name": "fake mono mic", "default_samplerate": 16000}
    playback = MagicMock()
    playback.is_playing.return_value = False
    ...
    check("real loop sends only usable finals, ignoring hello/empty/interim", sent == transcripts[1:3], repr(sent))
    ...
    check("new generation/playback echo rejected while queued caller audio survives", agent._echo_suppressed == 4)
...
async def test_capture_echo_overflow_and_response_filtering():
    ...
    agent.is_speaking = False
    agent._audio_callback(chunk(), 1280, None, None)
    check("capture gate blocks playback echo and its tail", agent.audio_q.empty() and agent._echo_suppressed == 2)
    agent._mic_resume_after = 0
    agent._audio_callback(chunk(), 1280, None, SimpleNamespace(input_overflow=True))
    agent._audio_callback(chunk(), 1280, None, None)
    check("capture evictions and device overflow both counted", agent._input_dropped == 1 and agent._input_overflows == 1)
    stamp, samples = agent.audio_q.get_nowait()
    check("capture packets carry a monotonic timestamp", stamp <= time.perf_counter() and len(samples) == 1280)
