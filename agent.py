...
class CallingAgent:
    ...
    def _audio_callback(self, indata, frames, time_info, status):
        ...
        # Gate at capture time: queued echo must not become caller speech when
        # playback has finished by the time the event loop drains the queue.
        if getattr(self, "disable_barge_in", False):
            active = self.is_speaking or bool(self.playback and self.playback.is_playing())
            if active:
                self._mic_resume_after = captured_at + ECHO_GUARD_SECONDS
            if active or captured_at < self._mic_resume_after:
            ...
            with in_stream:
                ...
                async with self.client.aio.live.connect(
                    ...
                    while self.is_running:
                        ...
                        # Process one capture block and honor its endpoint before
                        # consuming the next. Never merge a backlog across turns.
                        chunk_batch = []
                        capture_age_ms = 0.0
                        try:
                        ...
                        if chunk_batch:
                            ...
                            for chunk in chunk_batch:
                                ...
                                agent_active = not self.disable_barge_in and (
                                    self.is_speaking
                                    or (
